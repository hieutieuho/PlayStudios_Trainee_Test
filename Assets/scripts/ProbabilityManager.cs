...
public class ProbabilityManager : MonoBehaviour
{
    ...
    public List<float> getListProbability(List<M_Item> listItem){
        return listItem.Select(x => x.Probability).ToList<float>();
    }

    List<float> makeProbabilityRange(List<float> listProbability){
    ...
    public int getIndexByProbability(List<float> listProbability){
